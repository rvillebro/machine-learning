...
class Network():
    ...
    def __str__(self):
        ret_str = '{} with {} layer(s) and {} as loss function\n'.format(
            self.__class__.__name__, len(self._layers), self.loss_function)
...
class NeuralNetwork(Network):
    def __init__(self, loss_function: LossFunction = MSE()):
        ...
        if not isinstance(loss_function, LossFunction):
            raise TypeError('type {} not valid for argument loss_function. Please parse a subclass of LossFunction.'.format(
                type(loss_function)))
    ...
    def train(self, train_x, train_y, test_data=None, batch_size=10, epochs=1):
        ...
        for _ in range(epochs):
            ...
            for x, y in batch(train_x, train_y, batch_size):
                ...
                # d_E_x / d_output_x (d_afunc)
                self._layers[-1]._backprop_pass_on = self.loss_function.derivative(
                    y, outputs)
    ...
    def remove(self, layer):
        try:
            ...
            raise ValueError('{} not found in {} _layers'.format(
